...
import { CountryIndicatorDataResult } from "../ServerComponents/CountryIndicatorData"
import { InfoTitle } from "../AppGrid/AppHoverCard"

interface CompareChartProps {
    data1: CountryIndicatorDataResult | null
    data2: CountryIndicatorDataResult | null
    country1Name: string
    country2Name: string
...
// Reusing pyramid logic for a single country
const getPyramidData = (data: CountryIndicatorDataResult["data"]) => {
...
const CompareChart = ({ data1, data2, country1Name, country2Name, chartType }: CompareChartProps) => {
    // Merge standard data (bar, line, area)
    const mergedData = useMemo(() => {
        if (chartType === "pyramid") return []

        const map = new Map<string, any>()

        if (data1?.data) {
            data1.data.forEach(d => {
                map.set(d.timeLabel, { timeLabel: d.timeLabel, [country1Name]: d.value })
            })
        }

        if (data2?.data) {
            data2.data.forEach(d => {
                const existing = map.get(d.timeLabel) || { timeLabel: d.timeLabel }
                existing[country2Name] = d.value
                map.set(d.timeLabel, existing)
            })
        }

        return Array.from(map.values()).sort((a, b) => {
            return extractYear(a.timeLabel) - extractYear(b.timeLabel)
        })
    }, [data1, data2, country1Name, country2Name, chartType])

    const pyramidData1 = useMemo(() => getPyramidData(data1?.data), [data1])
    const pyramidData2 = useMemo(() => getPyramidData(data2?.data), [data2])

    if ((!data1?.data || data1.data.length === 0) && (!data2?.data || data2.data.length === 0)) {
        return (
    ...
    const title = data1?.title || data2?.title || "Compare Chart"
    const description = data1?.description || data2?.description || ""
    const topicName = data1?.topicName || data2?.topicName || ""
    ...
    const formatValue = (value: unknown) => {
        ...
        const isTargetIndicator = lowerTitle.includes("total population") ||
            ...
            lowerTitle.includes("migration rate") ||
    ...
    const renderPyramid = (data: PyramidDatum[], countryName: string) => (
        <div className="flex-1 min-w-[300px] h-full flex flex-col items-center w-full">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{countryName}</h3>
    ...
    const renderChart = () => {
        if (chartType === "pyramid") {
            return (
                <div className="flex flex-col lg:flex-row w-full h-full gap-4">
                    {renderPyramid(pyramidData1, country1Name)}
                    {renderPyramid(pyramidData2, country2Name)}
                </div>
            )
        }
        ...
        return (
            <ResponsiveContainer width="100%" height="100%">
                {chartType === "bar" ? (
                    <BarChart data={mergedData} margin={{ top: 0, right: 15, left: 20, bottom: 10 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border-3)" />
                        ...
                        <Tooltip
                            ...
                            formatter={(value: unknown, name: any) => [formatValue(value), name]}
                        />
                        <Legend verticalAlign="top" align="right" />
                        <Bar dataKey={country1Name} fill="var(--color-chart-5)" />
                        <Bar dataKey={country2Name} fill="var(--color-chart-7)" />
                    </BarChart>
                ) : chartType === "line" ? (
                    <LineChart data={mergedData} margin={{ top: 0, right: 15, left: 10, bottom: 10 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border-3)" />
                        <XAxis dataKey="timeLabel" tick={{ fontSize: 12 }} stroke="var(--color-text-3)">
                            <Label value="Year" offset={-5} className="font-semibold text-xs" position="insideBottom" fill="var(--color-text-3)" />
                        </XAxis>
                        ...
                        />
                        <Legend verticalAlign="top" align="right" />
                        <Line type="monotone" dataKey={country1Name} stroke="var(--color-chart-5)" strokeWidth={3} dot={{ fill: "var(--color-surface-base)", r: 4 }} activeDot={{ r: 5, stroke: "var(--color-surface-base)" }} />
                        <Line type="monotone" dataKey={country2Name} stroke="var(--color-chart-7)" strokeWidth={3} dot={{ fill: "var(--color-surface-base)", r: 4 }} activeDot={{ r: 5, stroke: "var(--color-surface-base)" }} />
                    </LineChart>
                ) : (
                    <AreaChart data={mergedData} margin={{ top: 0, right: 15, left: 20, bottom: 10 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border-3)" />
                        <XAxis dataKey="timeLabel" tick={{ fontSize: 12 }} stroke="var(--color-text-3)">
                            <Label value="Year" offset={-5} className="font-semibold text-xs" position="insideBottom" fill="var(--color-text-3)" />
                        </XAxis>
                        ...
                        />
                        <Legend verticalAlign="top" align="right" />
                        <Area type="monotone" dataKey={country1Name} stroke="var(--color-chart-5)" strokeWidth={3} fill="var(--color-chart-5)" fillOpacity={0.35} dot={{ fill: "var(--color-surface-base)", r: 4 }} activeDot={{ r: 5, stroke: "var(--color-surface-base)" }} />
                        <Area type="monotone" dataKey={country2Name} stroke="var(--color-chart-7)" strokeWidth={3} fill="var(--color-chart-7)" fillOpacity={0.35} dot={{ fill: "var(--color-surface-base)", r: 4 }} activeDot={{ r: 5, stroke: "var(--color-surface-base)" }} />
                    </AreaChart>
                )}
            </ResponsiveContainer>
        )
